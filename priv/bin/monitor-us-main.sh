...
us_main_hostname=$(echo "${us_monitor_base_content}" | grep us_main_hostname | sed 's|^[[:space:]]*{[[:space:]]*us_main_hostname,[[:space:]]*"||1' | sed 's|"[[:space:]]*}.$||1')
...
# extra one:
#
make -s us_main_monitor_exec EPMD_PORT=${erl_epmd_port} CMD_LINE_OPT="$* --config-file ${us_monitor_config_file} --target-cookie ${remote_vm_cookie}"
